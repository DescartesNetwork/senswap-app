...
Utils.getBalance = (address) => {
  return new Promise((resolve, reject) => {
    ...
    const publicKey = Utils.fromAddress(address);
...
Utils.getTokenAccountData = (address) => {
  return new Promise((resolve, reject) => {
    ...
    const accountPublicKey = Utils.fromAddress(address);
    return connection.getAccountInfo(accountPublicKey).then(({ data: accountData }) => {
      ...
      return connection.getAccountInfo(Utils.fromAddress(accountValue.token)).then(({ data: tokenData }) => {
...
Utils.newSRC20Account = (tokenPublicKey, payer) => {
  return new Promise((resolve, reject) => {
    const connection = Utils.createConnection();
    const { sol: { tokenFactoryAddress } } = configs;
    const programId = Utils.fromAddress(tokenFactoryAddress);
